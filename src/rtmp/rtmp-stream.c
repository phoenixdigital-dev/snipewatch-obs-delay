...
static void rtmp_stream_data_direct(void *data, struct encoder_packet *packet);

/* Pacer thread → original packet path (the output takes its own reference) */
static void swd_send_to_stream(void *ctx, struct encoder_packet *packet)
{
	rtmp_stream_data_direct(ctx, packet);
}

static const char *rtmp_stream_getname(void *unused)
{
	UNUSED_PARAMETER(unused);
	return obs_module_text("SnipeWatchDelayOutput");
}
...
static void rtmp_stream_destroy(void *data)
{
	...
	/* SnipeWatch: stop the pacer before tearing the output down */
	if (stream->delay) {
		swd_destroy(stream->delay);
		stream->delay = NULL;
	}
...
static void *rtmp_stream_create(obs_data_t *settings, obs_output_t *output)
{
	...
	stream->delay = swd_create(swd_send_to_stream, stream);
	return stream;
...
static void rtmp_stream_stop(void *data, uint64_t ts)
{
	...
	/* SnipeWatch: the delayed tail would never reach the graceful stop timestamp, stop now. */
	if (stream->delay) {
		swd_clear_running(stream->delay);
		swd_stop(stream->delay);
		ts = 0;
	}
...
static bool rtmp_stream_start(void *data)
{
	...
	/* SnipeWatch: fresh timeline, with the delay currently asked for */
	swd_start(stream->delay, swd_requested_delay());
	swd_set_running(stream->delay);
...
/* SnipeWatch: packets from the encoders go through the delay timeline first */
static void rtmp_stream_data(void *data, struct encoder_packet *packet)
{
	struct rtmp_stream *stream = data;

	if (!packet || !stream->delay) {
		rtmp_stream_data_direct(data, packet);
		return;
	}
	if (disconnected(stream) || !active(stream))
		return;
	swd_push(stream->delay, packet);
}
...
struct obs_output_info swd_output_info = {
	.id = "snipewatch_delay_output",
